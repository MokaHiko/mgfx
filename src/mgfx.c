...
typedef struct mgfx_draw {
  struct descriptor_sets {
    mgfx_uh uhs[MGFX_SHADER_MAX_DESCRIPTOR_BINDING];
    uint32_t dh_count;
...
static mgfx_sbh s_transform_storage_buffer;
static mgfx_uh s_u_transforms;

static mgfx_draw s_draws[MGFX_MAX_DRAW_COUNT];
static uint32_t s_draw_count = 0;

static transform_data s_transforms[MGFX_MAX_DRAW_COUNT];
static transform_data s_current_transform;

// Built in assets
mgfx_th MGFX_WHITE_TH;
mgfx_th MGFX_BLACK_TH;
mgfx_th MGFX_LOCAL_NORMAL_TH;
...
int mgfx_init(const mgfx_init_info* info) {
  ...
  s_transform_storage_buffer = mgfx_storage_buffer_create(
      NULL, sizeof(transform_data) * MGFX_MAX_DRAW_COUNT);
  s_u_transforms =
      mgfx_uniform_create("u_transforms", MGFX_UNIFORM_TYPE_STORAGE_BUFFER);
  mgfx_set_buffer(s_u_transforms, (mgfx_ubh){s_transform_storage_buffer.idx});
  ...
  dbg_ui_ph = mgfx_program_create_graphics(
      dbg_ui_vsh, dbg_ui_fsh,
      (mgfx_graphics_create_info){.name = "mgfx_debug_text"});
  ...
  dbg_ui_font_atlas_dh = mgfx_uniform_create(
  ...
  MGFX_WHITE_TH = mgfx_texture_create_from_memory(
      &texture_info, VK_FILTER_NEAREST, (uint8_t[4]){255, 255, 255, 255}, 4);

  MGFX_BLACK_TH = mgfx_texture_create_from_memory(
      &texture_info, VK_FILTER_NEAREST, (uint8_t[4]){0, 0, 0, 0}, 4);

  MGFX_LOCAL_NORMAL_TH = mgfx_texture_create_from_memory(
...
mgfx_ph mgfx_program_create_graphics(mgfx_sh vsh, mgfx_sh fsh,
                                     mgfx_graphics_create_info info) {
  program_entry* entry =
  ...
  entry->value.cull_mode = info.cull_mode;

  if (info.name.data != NULL) {
    memcpy(entry->value.name, info.name.c_str, mx_strlen(info.name));
  };
...
void mgfx_set_buffer(mgfx_uh uh, mgfx_ubh ubh) {
  descriptor_entry* entry;
  HASH_FIND(hh, s_descriptor_table, &uh, sizeof(uh), entry);
...
void mgfx_set_texture(mgfx_uh uh, mgfx_th th) {
  descriptor_entry* entry;
  HASH_FIND(hh, s_descriptor_table, &uh, sizeof(uh), entry);
  MX_ASSERT(entry != NULL, "Descriptor invalid handle!");
...
mgfx_uh mgfx_uniform_create_and_set_uniform_buffer(const char* name,
                                                   mgfx_ubh ubh) {
  mgfx_uh out = mgfx_uniform_create(name, MGFX_UNIFORM_TYPE_UNIFORM_BUFFER);
  mgfx_set_buffer(out, ubh);
  return out;
}

mgfx_uh mgfx_uniform_create_and_set_storage_buffer(const char* name,
                                                   mgfx_sbh sbh) {
  mgfx_uh out = mgfx_uniform_create(name, MGFX_UNIFORM_TYPE_STORAGE_BUFFER);
  mgfx_set_buffer(out, (mgfx_ubh){.idx = sbh.idx});
  return out;
}

mgfx_uh mgfx_uniform_create_and_set_texture(const char* name, mgfx_th th) {
  mgfx_uh out =
      mgfx_uniform_create(name, MGFX_UNIFORM_TYPE_COMBINED_IMAGE_SAMPLER);
  mgfx_set_texture(out, th);
  return out;
}

void mgfx_uniform_destroy(mgfx_uh uh) {}
...
void mgfx_bind_descriptor(mgfx_uh uh) {
  uint32_t ds_idx = 0;
  ...
  current_draw->desc_sets[ds_idx].uhs[descriptor_idx] = uh;
...
void mgfx_set_transform(const float* mtx) {
  memcpy(s_current_transform.transform.val, mtx, sizeof(float) * 16);
  s_current_transform.inverse_transform =
      mx_mat4_inverse(s_current_transform.transform);
}

void mgfx_set_view(const float* mtx) {
  memcpy(s_current_transform.view.val, mtx, sizeof(float) * 16);
  s_current_transform.inverse_view = mx_mat4_inverse(s_current_transform.view);
}

void mgfx_set_proj(const float* mtx) {
  memcpy(s_current_transform.proj.val, mtx, sizeof(float) * 16);
}

void mgfx_submit(uint8_t target, mgfx_ph ph) {
  if (s_draw_count >= MGFX_MAX_DRAW_COUNT) {
    MX_LOG_WARN("Reached max draws %u ! Consider instancing or batching!",
                MGFX_MAX_DRAW_COUNT);
    return;
  }

  if (ph.idx == MGFX_INVALID_HANDLE) {
    MX_LOG_WARN("Attempting to submit with invlid ph!");
    return;
  }

  // Bind mgfx built in descriptors.
  mgfx_bind_descriptor(s_u_transforms);
  ...
  for (int i = 0; i < 16; i++) {
    if (s_current_transform.transform.val[i] != 0) {
      valid_mtx = MX_TRUE;
    }
  }
...
#endif
  s_transforms[s_draw_count] = s_current_transform;
...
int mgfx_frame() {
  ...
  mgfx_buffer_update(s_transform_storage_buffer.idx, s_transforms,
  ...
  for (uint32_t draw_idx = 0; draw_idx < s_draw_count; draw_idx++) {
      ...
      for (uint32_t target_draw_idx = draw_idx; target_draw_idx < s_draw_count;
        ...
        for (uint32_t ds_idx = 0; ds_idx < MGFX_SHADER_MAX_DESCRIPTOR_SET;
          ...
          for (uint32_t bind_idx = 0;
            ...
            mgfx_uh uh = target_draw->desc_sets[ds_idx].uhs[bind_idx];
            const descriptor_entry* descriptor_entry;

            HASH_FIND(hh, s_descriptor_table, &uh, sizeof(mgfx_uh),
                      descriptor_entry);

            if (!descriptor_entry) {
              MX_LOG_WARN(
                  "[MGFX] Shader Program '%s' binding (%u) invalid handle!",
                  program_entry->value.name, bind_idx);
              MX_ASSERT(0);
            }

            if (!descriptor_entry->value.buffer &&
                !descriptor_entry->value.image) {
              MX_LOG_WARN("[MGFX] Descriptor entry is unset. Did you forget to "
                          "call mgfx_uniform_set__?");
              MX_ASSERT(0);
              return MX_FAILURE;
            }
    ...
    for (uint32_t ds_idx = 0; ds_idx < MGFX_SHADER_MAX_DESCRIPTOR_SET;
      ...
      uint32_t ds_hash;
      mx_murmur_hash_32(draw->desc_sets[ds_idx].uhs,
                        draw->desc_sets[ds_idx].dh_count * sizeof(mgfx_uh),
      ...
      for (uint32_t binding_idx = 0;
        ...
        mgfx_uh uh = draw->desc_sets[ds_idx].uhs[binding_idx];
        const descriptor_entry* descriptor_entry;
...
void mgfx_shutdown() {
  // Destroy built in
  mgfx_texture_destroy(MGFX_WHITE_TH, MX_TRUE);
  mgfx_texture_destroy(MGFX_BLACK_TH, MX_TRUE);
  mgfx_texture_destroy(MGFX_LOCAL_NORMAL_TH, MX_TRUE);

  mgfx_buffer_destroy(s_transform_storage_buffer.idx);
  mgfx_uniform_destroy(s_u_transforms);
  ...
  mgfx_uniform_destroy(dbg_ui_font_atlas_dh);
