...
const char *MX_LOG_COLOR_ERROR = "\033[0;31m";
const char *MX_LOG_COLOR_WARN = "\033[0;33m";
const char *MX_LOG_COLOR_RESET = "\033[0m";
...
vk_debug_callback(VkDebugUtilsMessageSeverityFlagBitsEXT msg_severity,
  ...
  case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
    printf("[%s]: %s\n", msg_type_strings[msg_type], pCallbackData->pMessage);
    break;
  case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
    printf("[%s]: %s\n", msg_type_strings[msg_type], pCallbackData->pMessage);
    break;
  case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
    printf("%s", MX_LOG_COLOR_WARN);
    printf("[%s]: %s\n", msg_type_strings[msg_type], pCallbackData->pMessage);
    printf("%s", MX_LOG_COLOR_RESET);
    break;
  case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
    printf("%s[%s]:", MX_LOG_COLOR_ERROR, msg_type_strings[msg_type]);
    printf("%s %s\n", MX_LOG_COLOR_WARN, pCallbackData->pMessage);
    printf("%s", MX_LOG_COLOR_RESET);
    break;
  default:
    break;
...
int swapchain_create(VkSurfaceKHR surface, uint32_t width, uint32_t height,
                     swapchain_vk *swapchain, MxArena *memory_arena) {
  ...
  if (swapchain->image_count > K_SWAPCHAIN_MAX_IMAGES) {
    printf("%s Requested swapchain image count (%d) larger than max capacity "
           "%d!\n",
           MX_LOG_COLOR_ERROR, swapchain->image_count, K_SWAPCHAIN_MAX_IMAGES);
    printf("%s", MX_LOG_COLOR_RESET);
    assert(0);
  }
  VkImage *images =
...
void texture_create_2d(uint32_t width, uint32_t height, VkFormat format,
                       VkImageUsageFlags usage, texture_vk *texture) {
  ...
  VK_CHECK(vmaCreateImage(s_allocator, &image_info, &alloc_info,
                          &texture->image.handle, &texture->allocation, NULL));
}

void texture_create_view(const texture_vk *texture, VkImageAspectFlags aspect,
                         VkImageView *view) {
...
void framebuffer_create(uint32_t color_attachment_count,
  ...
  for (int i = 0; i < color_attachment_count; i++) {
    ...
    texture_create_view(framebuffer->color_attachments[i],
  ...
  if (depth_attachment) {
    ...
    texture_create_view(framebuffer->depth_attachment,
...
void program_create_descriptor_sets(
    const program_vk *program, const VkDescriptorBufferInfo *ds_buffer_infos,
    const VkDescriptorImageInfo *ds_image_infos, VkDescriptorSet *ds_sets) {
  VkWriteDescriptorSet ds_writes[K_SHADER_MAX_DESCRIPTOR_SET] = {};
  uint32_t ds_write_count = 0;
  for (int stage_index = 0; stage_index < MGFX_SHADER_STAGE_COUNT;
       stage_index++) {
    ...
    for (int ds_index = 0; ds_index < shader->descriptor_set_count;
         ds_index++) {
      ...
      VK_CHECK(
          vkAllocateDescriptorSets(s_device, &alloc_info, &ds_sets[ds_index]));

      for (int binding_index = 0; binding_index < ds->binding_count;
           binding_index++) {
...
void program_create_compute(const shader_vk *cs, program_vk *program) {
  ...
  for (int ds_index = 0; ds_index < cs->descriptor_set_count; ds_index++) {
    ...
    flat_ds_layouts[flat_ds_layout_count] =
        program->ds_layouts[flat_ds_layout_count];
  ...
  VK_CHECK(vkCreatePipelineLayout(s_device, &pipeline_layout_info, NULL,
                                  &program->layout));
  ...
  VK_CHECK(vkCreateComputePipelines(s_device, NULL, 1, &info, NULL,
                                    &program->pipeline));
}

void program_create_graphics(const shader_vk *vs, const shader_vk *fs,
                             const framebuffer_vk *fb, program_vk *program) {
  ...
  VkPipelineVertexInputStateCreateInfo vertex_input_state_info = {
      ...
      .pVertexBindingDescriptions =
          program->shaders[MGFX_SHADER_STAGE_VERTEX]->vertex_bindings,
  ...
  VkPipelineRenderingCreateInfoKHR rendering_create_info = {
      ...
      .depthAttachmentFormat = fb->depth_attachment
                                   ? fb->depth_attachment->image.format
                                   : VK_FORMAT_UNDEFINED,
...
int mgfx_init(const mgfx_init_info *info) {
  ...
  if (choose_physical_device_vk(s_instance, k_req_device_ext_count,
                                k_req_device_ext_names, &s_phys_device,
...
void mgfx_frame() {
  ...
  if (s_buffer_to_buffer_copy_count > 0) {
    ...
    for (int i = 0; i < s_buffer_to_buffer_copy_count; i++) {
      ...
      if ((s_buffer_to_buffer_copy_queue[i].dst->usage &
           VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) ==
          VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) {
