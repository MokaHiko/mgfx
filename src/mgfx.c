...
const uint32_t k_ds_pool_sizes_count =
    sizeof(k_ds_pool_sizes) / sizeof(VkDescriptorPoolSize);
...
static buffer_to_buffer_copy_vk
    s_buffer_to_buffer_copy_queue[MGFX_MAX_FRAME_BUFFER_COPIES];
static uint32_t s_buffer_to_buffer_copy_count = 0;

static buffer_to_image_copy_vk
    s_buffer_to_image_copy_queue[MGFX_MAX_FRAME_BUFFER_COPIES];
...
void image_update(const void *data, size_t size, image_vk *image) {
  ...
  memcpy((uint8_t*)bound_memory + s_image_staging_buffer_offset, data, size);
...
int swapchain_create(VkSurfaceKHR surface, uint32_t width, uint32_t height,
                     swapchain_vk *swapchain, mx_arena *memory_arena) {
  ...
  if (s_queue_indices[MGFX_QUEUE_GRAPHICS] !=
      s_queue_indices[MGFX_QUEUE_PRESENT]) {
  ...
  VK_CHECK(vkCreateSwapchainKHR(s_device, &swapchain_info, NULL,
                                &swapchain->handle));

  vkGetSwapchainImagesKHR(s_device, swapchain->handle, &swapchain->image_count,
                          NULL);
  ...
  VkImage *images =
      mx_arena_push(local_arena, swapchain->image_count * sizeof(VkImage));
  vkGetSwapchainImagesKHR(s_device, swapchain->handle, &swapchain->image_count,
                          images);
  ...
  for (uint32_t i = 0; i < swapchain->image_count; i++) {
    ...
    vkCreateImageView(s_device, &sc_img_view_info, NULL,
...
void buffer_update(buffer_vk *buffer, size_t buffer_offset, size_t size,
  ...
  if ((memory_flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) != 0) {
    ...
    memcpy((uint8_t*)staging_data + dst_offset, data, size);
  ...
  // TODO: Make staging buffer ring buffer
  mgfx_buffer_slice staging_buffer = buffer_allocate_from_primary(&s_staging_buffer_pool, size);
  buffer_update(&s_staging_buffer_pool, staging_buffer.offset,
...
void vertex_buffer_create(size_t size, const void *data,
                          vertex_buffer_vk *buffer) {
  buffer_create(size,
...
void index_buffer_create(size_t size, const void *data,
...
void uniform_buffer_create(size_t size, const void *data,
...
mgfx_buffer_slice buffer_allocate_from_primary(ring_buffer_vk *primary,
                                               size_t size) {
  VmaAllocationInfo alloc_info = {0};
  vmaGetAllocationInfo(s_allocator, primary->allocation, &alloc_info);

  size_t free_size = alloc_info.size - primary->head;
  MX_ASSERT(size <= free_size,
            "[BufferSliceAlloc] allocation failed. Must Reallocating!");

  size_t offset = primary->head;
...
void shader_create(size_t length, const char *code, shader_vk *shader) {
  ...
  if (module.shader_stage == SPV_REFLECT_SHADER_STAGE_VERTEX_BIT) {
    ...
    if (input_count > 0) {
      SpvReflectInterfaceVariable **input_variables = mx_arena_push(
          &shader_arena, input_count * sizeof(SpvReflectInterfaceVariable));
  ...
  if (binding_count > 0) {
    MX_ASSERT(result == SPV_REFLECT_RESULT_SUCCESS);
    SpvReflectDescriptorBinding **bindings = mx_arena_push(
        &shader_arena, binding_count * sizeof(SpvReflectDescriptorBinding *));
  ...
  MX_ASSERT(result == SPV_REFLECT_RESULT_SUCCESS);
  if (pc_count > 0) {
    SpvReflectBlockVariable **push_constants = mx_arena_push(
        &shader_arena, pc_count * sizeof(SpvReflectBlockVariable));
...
void pipeline_create_graphics(const shader_vk *vs, const shader_vk *fs,
                              const framebuffer_vk *fb, mgfx_program *program) {
  MX_ASSERT(vs != NULL,
  ...
  for (int stage_idx = 0; stage_idx < 2; stage_idx++) {
    ...
    for (int ds_idx = 0; ds_idx < shader->ds_count; ds_idx++) {
      ...
      for (uint32_t binding = 0; binding < ds->binding_count; binding++) {
        ...
        if (ds_bindings->stageFlags != 0) {
          ds_bindings->stageFlags |= stage_idx == 0
    ...
    for (int pc_index = 0; pc_index < shader->pc_count; pc_index++) {
      flat_pc_ranges[flat_pc_range_count] =
          shader->pc_ranges[flat_pc_range_count];
      ++flat_pc_range_count;
    }
  }
...
mx_bool swapchain_update(const frame_vk *frame, int width, int height,
  ...
  if (sc->resize == MX_TRUE) {
    ...
    if (s_surface_caps.currentExtent.width == UINT32_MAX) {
      width = (int)mx_clampf((float)width, (float)s_surface_caps.minImageExtent.width,
                        (float)s_surface_caps.maxImageExtent.width);
      height = (int)mx_clampf((float)height, (float)s_surface_caps.minImageExtent.height,
...
int mgfx_init(const mgfx_init_info *info) {
  ...
  VkExtensionProperties *avail_props = mx_arena_push(
      &vk_init_arena, avail_prop_count * sizeof(VkExtensionProperties));
  ...
  VkLayerProperties *layer_props = mx_arena_push(
      &vk_init_arena, sizeof(VkLayerProperties) * layer_prop_count);
  ...
  if (choose_physical_device_vk(s_instance, k_req_device_ext_count,
                                ...
                                &s_phys_device, &vk_init_arena) != VK_SUCCESS) {
    MX_LOG_ERROR("Failed to find suitable physical device!");
    mx_arena_free(&vk_init_arena);
    return -1;
  }
  MX_LOG_INFO("Device : %s (%d)", s_phys_device_props.deviceName,
              s_phys_device_props.deviceType);
  ...
  VkSurfaceFormatKHR *surface_fmts = mx_arena_push(
      &vk_init_arena, surface_fmt_count * sizeof(VkSurfaceFormatKHR));
  vkGetPhysicalDeviceSurfaceFormatsKHR(s_phys_device, s_surface,
  ...
  VkPresentModeKHR *present_modes = mx_arena_push(
      &vk_init_arena, surface_fmt_count * sizeof(VkPresentModeKHR));
  vkGetPhysicalDeviceSurfacePresentModesKHR(s_phys_device, s_surface,
  ...
  debug_ui_vbh_pool = mgfx_vertex_buffer_create(NULL, MGFX_DEBUG_MAX_TEXT * 4 *
                                                          sizeof(glyph_vertex));
  tvb_pool.vb = debug_ui_vbh_pool;
  tvb_pool.size = MGFX_DEBUG_MAX_TEXT * 4 * sizeof(glyph_vertex);

  debug_ui_ibh_pool = mgfx_index_buffer_create(NULL, MGFX_DEBUG_MAX_TEXT * 6 *
                                                         sizeof(uint32_t));
  mx_free(ttf_buffer);
...
mgfx_transient_buffer mgfx_transient_buffer_allocate(mx_ptr_t buffer_idx,
                                                     const void *data,
                                                     size_t len) {
  buffer_entry *pool_entry = mx_alloc(sizeof(buffer_entry), 0);
  HASH_FIND(hh, s_buffer_table, &buffer_idx, sizeof(mx_ptr_t), pool_entry);

  MX_ASSERT(pool_entry, "Transient buffer pool is NULL!");

  mgfx_transient_buffer tb =
      buffer_allocate_from_primary(&pool_entry->value, len);
...
mgfx_tbh mgfx_transient_vertex_buffer_allocate(const void *data, size_t len) {
  ...
  tvb_pool.head = (uint32_t)((tvb_pool.head + len) % tvb_pool.size);
...
mgfx_ph mgfx_program_create_(mgfx_sh *sh, uint32_t sh_count,
                             int32_t primitive_topology, int32_t polygon_mode) {
...
void mgfx_set_transform(const float *mtx) {
  memcpy(s_current_transform, mtx, sizeof(float) * 16);
}

void mgfx_set_view(const float *mtx) {
  memcpy(s_current_view, mtx, sizeof(float) * 16);
}

void mgfx_set_proj(const float *mtx) {
  memcpy(s_current_proj, mtx, sizeof(float) * 16);
}
...
void mgfx_submit(uint8_t target, mgfx_ph ph) {
  ...
  current_draw->sort_key =
      ((uint64_t)(target) << 56) | // highest priority (view)
...
void mgfx_frame() {
  ...
  if (s_buffer_to_buffer_copy_count > 0) {
    ...
    for (uint32_t copy_idx = 0; copy_idx < s_buffer_to_buffer_copy_count;
      ...
      vkCmdCopyBuffer(frame->cmd,
                      ...
                      s_buffer_to_buffer_copy_queue[copy_idx].dst->handle, 1,
                      &s_buffer_to_buffer_copy_queue[copy_idx].copy);

      if ((s_buffer_to_buffer_copy_queue[copy_idx].dst->usage &
           VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) ==
          VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) {
      ...
      if ((s_buffer_to_buffer_copy_queue[copy_idx].src->allocation_type ==
         ...
         * {*/
        /*    VmaAllocationInfo alloc_info = {};*/
        /*    vmaGetAllocationInfo(s_allocator, buffer_pool->value.allocation,
         * &alloc_info);*/
        /*    buffer_pool->value.tail =*/
        /*        buffer_pool->value.tail +
         * s_buffer_to_buffer_copy_queue[copy_idx].src->slice_size;*/
        /*}*/
      }
    };
    ...
    vkCmdPipelineBarrier(frame->cmd, VK_PIPELINE_STAGE_TRANSFER_BIT,
                         ...
                         vb_copy_barrier_count, vb_copy_barriers, 0, NULL, 0,
                         NULL);
  ...
  for (uint32_t draw_idx = 0; draw_idx < s_draw_count; draw_idx++) {
    ...
    if (draw->view_target != target) {
      ...
      if (cur_program != &program_entry->value) {
        ...
        vkCmdBindPipeline(frame->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          (VkPipeline)cur_program->pipeline);
      }
      ...
      for (uint32_t ds_idx = 0; ds_idx < MGFX_SHADER_MAX_DESCRIPTOR_SET;
        ...
        for (uint32_t binding_idx = 0;
             binding_idx < draw->descriptor_sets[ds_idx].dh_count;
             binding_idx++) {
      ...
      for (uint32_t color_attachment_idx = 0;
        ...
        vk_cmd_clear_image(frame->cmd, fb->color_attachments[0], &range,
                           &clear);
      }
    ...
    for (uint32_t ds_idx = 0; ds_idx < MGFX_SHADER_MAX_DESCRIPTOR_SET;
      ...
      mx_murmur_hash_32(draw->descriptor_sets[ds_idx].dhs,
                        draw->descriptor_sets[ds_idx].dh_count *
                            sizeof(mgfx_dh),
    ...
    if (flat_ds_count > 0) {
      vkCmdBindDescriptorSets(frame->cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                              (VkPipelineLayout)cur_program->pipeline_layout, 0,
                              flat_ds_count, flat_ds, 0, NULL);
    }
    ...
    if (draw->vbh_count > 0 && cur_vbs[0] != (VkBuffer)draw->vbhs[0].idx) {
      ...
      vkCmdBindVertexBuffers(frame->cmd, 0, draw->vbh_count,
                             (VkBuffer *)draw->vbhs, &offsets);
    }

    mx_bool transient_buffer_overflow = MX_FALSE;

    for (uint32_t tvbh_idx = 0; tvbh_idx < draw->tvbh_count; ++tvbh_idx) {
      uint64_t tb_idx = draw->tvbhs[tvbh_idx].idx;
      ...
      vkCmdBindVertexBuffers(frame->cmd, 0, 1, (VkBuffer *)&tvb_pool.vb,
                             &offsets);
    ...
    if (draw->tib.buffer_handle != (uint64_t)(VK_NULL_HANDLE)) {
      ...
      const buffer_entry *index_buffer_entry;
      HASH_FIND(hh, s_buffer_table, &cur_ib, sizeof(mgfx_ibh),
                index_buffer_entry);
      ...
      vkCmdBindIndexBuffer(frame->cmd, cur_ib, draw->tib.offset,
                           VK_INDEX_TYPE_UINT32);
    }

    vkCmdPushConstants(
        frame->cmd, (VkPipelineLayout)cur_program->pipeline_layout,
        VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(draw->draw_pc), &draw->draw_pc);

    if (cur_ib) {
      vkCmdDrawIndexed(frame->cmd, 66, 1, 0, 0, 0);
    } else {
  ...
  VK_CHECK(vkQueueSubmit(s_queues[MGFX_QUEUE_GRAPHICS], 1, &submit_info,
                         frame->render_fence));
  ...
  VkResult present_result =
      vkQueuePresentKHR(s_queues[MGFX_QUEUE_GRAPHICS], &present_info);
...
void mgfx_debug_draw_text(uint32_t x, uint32_t y, const char *fmt, ...) {
  ...
  for (size_t char_idx = 0; char_idx < strlen(word_buffer); char_idx++) {
    ...
    uint32_t char_indices[6] = {
        0 + vertex_offset, 3 + vertex_offset,
        2 + vertex_offset, // Triangle 1: bottom-left → top-left → top-right
        0 + vertex_offset, 2 + vertex_offset,
  ...
  mgfx_tbh tvb = mgfx_transient_vertex_buffer_allocate(
      vertices, vertex_count * sizeof(glyph_vertex));

  static mgfx_transient_buffer tib = {0};
  if ((VkBuffer)tib.buffer_handle == VK_NULL_HANDLE) {
    tib = mgfx_transient_buffer_allocate(debug_ui_ibh_pool.idx, indices,
