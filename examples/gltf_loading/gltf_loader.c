...
static void mikk_get_position(const SMikkTSpaceContext* ctx, float fv_pos_out[], const int iface, const int ivert) {
    const struct primitive* primitive = ctx->m_pUserData;
    ...
    fv_pos_out[0] = position[0]; fv_pos_out[1] = position[1]; fv_pos_out[2] = position[2];
}

static void mikk_get_normal(const SMikkTSpaceContext* ctx, float fv_norm_out[], const int iface, const int ivert) {
    const struct primitive* primitive = ctx->m_pUserData;
    const uint32_t idx = primitive->indices[iface * 3 + ivert];
    ...
    fv_norm_out[0] = normal[0]; fv_norm_out[1] = normal[1]; fv_norm_out[2] = normal[2];
}

static void mikk_get_tex_coord(const SMikkTSpaceContext* ctx, float fv_texc_out[], const int iface, const int ivert) {
    const struct primitive* primitive = ctx->m_pUserData;
    const uint32_t idx = primitive->indices[iface * 3 + ivert];
    ...
    fv_texc_out[0] = texc[0]; fv_texc_out[1] = texc[1];
}

static void mikk_set_tspace_basic(const SMikkTSpaceContext* ctx, const float fv_tangent[],
    ...
    tangent[0] = fv_tangent[0]; tangent[1] = fv_tangent[1]; tangent[2] = fv_tangent[2] ;
    tangent[3] = fsign;
}
...
void load_scene_from_path(const char* path,
        ...
        for(size_t i = 0; i < data->materials_count; i++) {
            ...
            if(mat->has_pbr_metallic_roughness) {
                ...
                if(mat->pbr_metallic_roughness.base_color_texture.texture) {
                    ...
                    if(scene->textures[tex_idx].image.format == VK_FORMAT_UNDEFINED) {
                        ...
                        load_texture_2d_from_path(absolute_path, VK_FORMAT_R8G8B8A8_SRGB, &scene->textures[tex_idx]);
                        MX_LOG_SUCCESS("Texture loaded: %s (%u mb)!",
                                       data->textures[tex_idx].image->uri,
                                       scene->textures[tex_idx].image.extent.width * scene->textures[tex_idx].image.extent.height * 4 / 8 / MX_MB);
                    }
                ...
                if(mat->has_emissive_strength) {
                    scene->materials[i].properties.emissive_strength = mat->emissive_strength.emissive_strength;
                } else {
                ...
                if(mat->pbr_metallic_roughness.metallic_roughness_texture.texture) {
                    ...
                    if(scene->textures[tex_idx].image.format == VK_FORMAT_UNDEFINED) {
                        ...
                        load_texture_2d_from_path(absolute_path, VK_FORMAT_R8G8B8A8_UNORM, &scene->textures[tex_idx]);
                        MX_LOG_SUCCESS("Texture loaded: %s (%u mb)!",
                                       data->textures[tex_idx].image->uri,
                                       scene->textures[tex_idx].image.extent.width * scene->textures[tex_idx].image.extent.height * 4 / 8 / MX_MB);
                    }
        ...
        for (size_t primitive_idx = 0; primitive_idx < mesh->primitives_count; primitive_idx++) {
            ...
            for (size_t attrib_idx = 0; attrib_idx < primitive->attributes_count; attrib_idx++) {
                ...
                if (attribute->type == cgltf_attribute_type_position) {
                    ...
                    mesh_primitive->vertices = mx_arena_push( &scene->allocator, mesh_primitive->vertex_count * vertex_size);
                    break;
                }
            }
            ...
            for (size_t attrib_idx = 0; attrib_idx < primitive->attributes_count; attrib_idx++) {
                ...
                for (size_t attrib_data_idx = 0; attrib_data_idx < attribute->data->count; attrib_data_idx++) {
                    memcpy((uint8_t*)mesh_primitive->vertices + (attrib_data_idx * vertex_size) + attrib_offset,
                           attrib_buffer + (attrib_data_idx * attribute->data->stride), attribute->data->stride);
                }
            ...
            if (primitive->indices) {
                ...
                mesh_primitive->indices = mx_arena_push(&scene->allocator, primitive->indices->count * sizeof(uint32_t));
            ...
            vertex_buffer_create(mesh_primitive->vertex_count * scene->vl->stride,
                                 mesh_primitive->vertices, &mesh_primitive->vb);

            index_buffer_create(mesh_primitive->index_count * sizeof(uint32_t),
                                mesh_primitive->indices, &mesh_primitive->ib);

            MX_LOG_SUCCESS("Mesh loaded: (%u vertices, %d indices) %s",
                           mesh_primitive->vertex_count,
                           mesh_primitive->index_count, mesh->name ? mesh->name : "<unamed>");
        }
    }
    ...
    if((flags & gltf_loader_flag_materials) == gltf_loader_flag_materials) {
        for(int mat_idx = 0; mat_idx < scene->material_count; mat_idx++) {
            ...
            program_create_descriptor_set(gfx_program, &mat->ds);

            uniform_buffer_create(sizeof(mat->properties), &mat->properties, &mat->properties_buffer);

            descriptor_buffer_create(&mat->properties_buffer, &mat->u_properties_buffer);
            descriptor_write(mat->ds, 0, &mat->u_properties_buffer);

            descriptor_texture_create(mat->albedo_texture, &mat->u_albedo_texture);
            descriptor_write(mat->ds, 1, &mat->u_albedo_texture);

            descriptor_texture_create(mat->metallic_roughness_texture, &mat->u_metallic_roughness_texture);
            descriptor_write(mat->ds, 2, &mat->u_metallic_roughness_texture);

            descriptor_texture_create(mat->normal_texture, &mat->u_normal_texture);
            descriptor_write(mat->ds, 3, &mat->u_normal_texture);

            descriptor_texture_create(mat->occlusion_texture, &mat->u_occlusion_texture);
            descriptor_write(mat->ds, 4, &mat->u_occlusion_texture);

            descriptor_texture_create(mat->emissive_texture, &mat->u_emissive_texture);
            descriptor_write(mat->ds, 5, &mat->u_emissive_texture);
        }
    }
...
void scene_destroy(mgfx_scene* scene) {
    ...
    for(uint32_t i = 0; i < scene->material_count ; i++) {
        buffer_destroy(&scene->materials[i].properties_buffer);
    }
    ...
    for(uint32_t i = 0; i < scene->mesh_count; i++) {
        for(size_t primitive_idx = 0; primitive_idx < scene->meshes[i].primitive_count; primitive_idx++) {
            buffer_destroy(&scene->meshes[i].primitives[primitive_idx].vb);
            buffer_destroy(&scene->meshes[i].primitives[primitive_idx].ib);
        }
    }
}
