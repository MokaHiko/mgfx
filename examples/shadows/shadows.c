...
directional_light dir_light = {.color = {1.0f, 1.0f, 1.0f, 1.0f}, .direction = {1.0f, -1.0f, 1.0f, 1.0f}};
...
void mgfx_example_update() {
    ...
    if(mgfx_get_key(GLFW_KEY_E)) {
        ...
        angle += 3.1415925 * MGFX_TIME_DELTA_TIME * 0.01f;
        glm_rotate(rotate_matrix, angle, (vec3){0, 0, 1});
        glm_mat4_mulv3(rotate_matrix, dir_light.direction, 1.0f, dir_light.direction);
